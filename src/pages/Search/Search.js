  ...
  useEffect(() => {
    ...
    if (access_token && searchQuery) {
      ...
      spotify(access_token)
        .get(
          `search?q=${q}&type=album,artist,playlist,track&market=from_token&limit=5`
        )
        .then((res) => {
          if (mounted) {
  ...
  return (
    <Box>
      ...
      <List className={classes.list}>
        {tracks.length > 0 && <ListSubheader>Tracks</ListSubheader>}
        {tracks.map((track, i) => (
        ...
        {artists.length > 0 && <ListSubheader>Artists</ListSubheader>}
        ...
        {playlists.length > 0 && <ListSubheader>Playlists</ListSubheader>}
